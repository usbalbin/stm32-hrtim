...
pub trait HrSlaveTimerCpt: HrSlaveTimer {
    ...
    fn split_capture(self) -> TimerSplitCapture<Self::Timer, Self::Prescaler, Ch1, Ch2>;
}

impl<TIM: Instance, PSCL: HrtimPrescaler, CPT1, CPT2> HrTimer for HrTim<TIM, PSCL, CPT1, CPT2> {
    type Prescaler = PSCL;
    type Timer = TIM;
...
impl<TIM: Instance, PSCL, CPT1, CPT2> HrTim<TIM, PSCL, CPT1, CPT2> {
...
impl<TIM: InstanceX, PSCL: HrtimPrescaler, CPT1, CPT2> HrSlaveTimer
...
impl<TIM: InstanceX, PSCL: HrtimPrescaler> HrSlaveTimerCpt
    for HrTim<
        TIM,
        PSCL,
    ...
    HrCapt<TIM, PSCL, Ch2, capture::NoDma>: HrCapture,
{
    ...
    fn split_capture(self) -> TimerSplitCapture<TIM, PSCL, Ch1, Ch2> {
        let HrTim {
            _timer,
            _prescaler,
            capture_ch1,
...
/// Timer Period event
impl<TIM: InstanceX, DST, PSCL, CPT1, CPT2> super::event::EventSource<DST, PSCL>
    for HrTim<TIM, PSCL, CPT1, CPT2>
{
    // $rstXr
    const BITS: u32 = 1 << 2;
